// Generated by Copilot: align API generator output path with existing imports and fix strict unknown invoke typing
...
export class WardrobeService {
  ...
  async initializeData() {
    try {
      const [items, profile] = await Promise.all([
        this.api.invoke(getItems) as Promise<WardrobeItemDto[]>,
        this.api.invoke(getProfile) as Promise<UserProfileDto>,
      ]);
  ...
  async addItem(item: CreateWardrobeItemRequest): Promise<CreateWardrobeItemResponse> {
    try {
      const response = (await this.api.invoke(addItem, { body: item })) as CreateWardrobeItemResponse;
      if (response.item) {
  ...
  async analyzeItem(image: string): Promise<WardrobeValidationDto> {
    return this.api.invoke(analyzeItem, { body: { image } }) as Promise<WardrobeValidationDto>;
  }

  async deleteItem(id: string) {
    await (this.api.invoke(deleteItem, { id }) as Promise<unknown>);
    this.items.update((current) => current.filter((i) => i.id !== id));
  }

  async deleteItems(itemIds: string[]) {
    await (this.api.invoke(deleteItems, { body: { itemIds } }) as Promise<unknown>);
  ...
  async updateProfile(request: UpdateProfileRequest) {
    const updated = (await this.api.invoke(updateProfile, { body: request })) as UserProfileDto;
    this.userProfile.set(updated);
  }

  async saveAnalysis(analysis: SeasonAnalysisResponse) {
    const updated = (await this.api.invoke(saveSeasonProfile, { body: analysis })) as UserProfileDto;
    this.userProfile.set(updated);
  }
}
